...
describe 'confluence::autotune' do
  context 'When autotune type is dedicated' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['confluence']['autotune']['enabled'] = true
        node.normal['confluence']['autotune']['type'] = 'dedicated'
  ...
  context 'When autotune type is shared' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['confluence']['autotune']['enabled'] = true
        node.normal['confluence']['autotune']['type'] = 'shared'
        node.automatic['memory']['total'] = '8011076kB'
      end.converge(described_recipe)
    end
  ...
  context 'When autotune type is mixed' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['confluence']['autotune']['enabled'] = true
        node.normal['confluence']['autotune']['type'] = 'mixed'
        node.automatic['memory']['total'] = '8011076kB'
      end.converge(described_recipe)
    end
